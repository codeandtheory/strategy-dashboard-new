...
export default function NewsPage() {
  ...
  return (
    <div className={`${getBgClass()} ${mode === 'code' ? 'font-mono' : 'font-[family-name:var(--font-raleway)]'}`}>
      <div className="mb-8">
        <h1 className={`text-4xl font-black uppercase ${getTextClass()} mb-2`}>News</h1>
        <p className={`${getTextClass()}/70 font-normal`}>Manage team announcements and updates.</p>
      </div>

      <Card className={`${getBgClass()} border ${getBorderClass()} p-6 ${getRoundedClass('rounded-xl')}`}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Newspaper className={`w-6 h-6 ${getTextClass()}`} />
            <h2 className={`text-2xl font-black uppercase ${getTextClass()}`}>News Management</h2>
          </div>
          <Button
        ...
        <div className={`text-center py-12 ${getTextClass()}/70`}>
          <p className="text-lg">News management coming soon.</p>
        </div>
      </Card>
    </div>
  )
